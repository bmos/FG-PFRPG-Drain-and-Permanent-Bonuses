...
local function handleRacialAbilities_new(nodeChar, sText)
    ...
    while aWords[i] do
        if StringManager.isNumberString(aWords[i]) then
            ...
            if nMod ~= 0 then
                ...
                elseif StringManager.contains(DataCommon.abilities,
                                              aWords[i - 1]) then
                    aIncreases[aWords[i - 1]] = nMod;
                else
                    local j = i + 1;
                    ...
                    if StringManager.isPhrase(aWords, j,
                                              {"to", "one", "ability", "score"}) then
                        bChoice = true;
                    end
                end
            end
    ...
    for k, v in pairs(aIncreases) do
        if StringManager.contains(DataCommon.abilities, k) then
            ...
            local nodeAbilityPerm = nodeAbility.createChild("abilperms")
                                        .createChild()
            DB.setValue(nodeAbilityPerm, "name", "string",
    ...
    if bChoice then
        ...
        local sMessage = Interface.getString(
                             "char_message_selectabilityincrease");
        wSelect.requestSelection(sTitle, sMessage, aAbilities,
                                 CharManager.onRaceAbilitySelect, nodeChar, 1);
        bApplied = true;
    end
